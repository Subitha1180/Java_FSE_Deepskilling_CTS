...
DROP PROCEDURE IF EXISTS MarkVIPCustomers;
DELIMITER $$
...
BEGIN
    ...
    VALUES (p_id, p_name, p_age, p_balance, p_is_vip, p_interest_rate);
END $$
DELIMITER ;

CALL AddCustomer(5, 'Eve', 55, 14000.00, 'N', 7.2);

DROP PROCEDURE IF EXISTS ShowVIPWithLoans;
DELIMITER $$
CREATE PROCEDURE ShowVIPWithLoans()
BEGIN
    SELECT 
        c.customer_id, c.name, c.is_vip,
        l.loan_id, l.due_date, l.interest_rate
    FROM 
        customers c
    JOIN 
...
CALL ShowVIPWithLoans();

DROP PROCEDURE IF EXISTS LoanSummary;
DELIMITER $$
CREATE PROCEDURE LoanSummary()
BEGIN
    SELECT 
        c.customer_id,
        c.name,
